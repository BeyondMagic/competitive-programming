...
auto number_digits (size_t number)
{
  size_t digits = 0;
...
size_t kamenetsky_fibonacci (size_t number)
{
  if (number <= 1)
    return 1;

  double x = (
    log10(double(number) / M_E) * double(number) +
    log10(2.0 * M_PI            * double(number)) / 2.0
  );

  return size_t(x) + 1;
}

auto solve (size_t nth)
{
  return number_digits(factorial(nth));
}
...
int main()
{
  ...
  while (a--)
  {
    size_t b;
    cin >> b;
    cout << kamenetsky_fibonacci(b) << endl;
  }
}
