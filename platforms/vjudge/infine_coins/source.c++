...
namespace math
{
	...
	namespace prime
	{
		...
		// Note: change the following variable's value to any other positive one.
		//       bigger the number: bigger the binary size.
		static constexpr const size_t MAX_NUMBERS = 100'000;
		constexpr auto Sieve_of_Eratosthenes()
		{
			// Plus one because we don't want to count the zero!...
			bitset<MAX_NUMBERS + 1> table;

			// Set whole table with ones.
			table.set();

			// Define initial non-primes numbers that are not used in the Sieve.
			table[0] = table[1] = false;

			for (size_t i = 2; i <= MAX_NUMBERS; i++)
				if (table[i])
					for (size_t j = i * i; j <= MAX_NUMBERS; j += i)
						table[j] = false;
...
/* calculation */
auto solve (intmax_t a)
{
	static constexpr auto primes = math::prime::Sieve_of_Eratosthenes();
	return primes[a] ? "Yes" : "No";
}
