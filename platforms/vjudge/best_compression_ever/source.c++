...
namespace math
{
  ...
  namespace base
  {
    ...
    int64_t base_to_decimal (string str, int64_t base)
    {
      ...
      {
        const auto answer = decimal_value(str[size_t(i)]);
         if (answer >= base)
            return -1;
         num += answer * power;
         power = power * base;
    ...
    string decimal_to_base (int64_t number, int64_t base)
    {
      ...
      while (number)
      {
        const auto value = base_value(char(number % base));
        if (value >= base + '0')
          return "";
        answer += value;
        number /= base;
...
/* calculation */
auto solve (uint64_t number, uint64_t to_base)
{
  if ((uint64_t(1) << (to_base + 1)) > number)
    return "yes";
  return "no";
}
