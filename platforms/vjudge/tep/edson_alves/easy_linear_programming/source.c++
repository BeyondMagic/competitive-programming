...
/* calculation */
auto solve (intmax_t a, intmax_t b, [[maybe_unused]] intmax_t c, intmax_t k)
{
    intmax_t sum = 0;

	// if they are equal, obviously has to be either k or a
	sum += min(a, k);
	k -= min(a, k);

	// if there are no ones greater than k, then it can't be summed any
	k -= min(b, k);
	sum -= k;
