...
public:
	...
	long long value_at(long long i) { return RSQ(i); }
...
{
	private:
		std::vector<long long> size, ps;

	public:
		UFDS(long long N) : size(N + 1, 1), ps(N + 1)
	{
		// ps = {0, 1, 2, 3, ..., N}
		std::iota(ps.begin(), ps.end(), 0);
	}

		long long find_set(long long x) const
		{
		...
		void union_set(long long x, long long y)
		{
			...
			long long p = find_set(x);
			long long q = find_set(y);
