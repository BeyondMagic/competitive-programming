...
/* calculation */
auto solve (math::matrix::matrix &map_a, math::matrix::matrix &map_b)
{
  if (math::matrix::is_equal(map_a, map_b))
    return SUCCESS;

  for (size_t i = 0; i < h; i++)
  {
    math::matrix::shift::down(map_a);

    if (math::matrix::is_equal(map_a, map_b))
      return SUCCESS;

    const auto size = map_a.at(i).size();
    for (size_t j = 0; j < size; j++)
    {
      math::matrix::shift::left(map_a);
...
int main()
{
  ...
  /* input */
  cin >> h >> w;
  math::matrix::matrix map_a(h), map_b(h);
  for (size_t i = 0; i < h; ++i)
    cin >> map_a.at(i);
